...
function setup(){const m=new Main();m.root={removeAllChildren(){}};m.make=()=>({active:false,removeAllChildren(){}});m.panel=m.artwork=()=>{};m.text=()=>({});m.button=()=>{};m.buildUI=m.resetView=m.draw=m.updateHUD=()=>{};m.showModal=kind=>assert.fail('unexpected modal '+kind);m.levels=[level,level];return m;}
